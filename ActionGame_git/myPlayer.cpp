		...
		// キーボードでの操作.
		{
			...
			if (pInput->IsPushKey(DIK_C) && m_meter == 5)
			{
				...
				if (pBulletOne != NULL)
				{
					...
					GetSoundInst()->PlaySE(SE_PLAYERBULLET);
				}
				m_meter = 0;
			}
		}
...
void Player::Show() {
	...
	if (HP > 0) {
		...
		// プレイヤーの描画.
		D3DXVECTOR3 pos(m_pos.x, m_pos.y + 1 * SIZE, 0);
		D3DXVECTOR3 weaponpos(m_weapon_pos.x, m_weapon_pos.y + 1 * SIZE, 0);
		RECT rc = { 0, 0, 0, 0 };
